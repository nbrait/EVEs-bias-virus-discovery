...
## reading in tree
tree <- read.tree("PA_tree.contree") # insert nwk tree with bootstrap
## replacing the label descriptions
replace_words <- read.csv("PA_references.txt", sep = "\t", header = FALSE, col.names = c("Accession", "Virus"))
...
tree <- root(tree, node = 96, edgelabel = TRUE)
...
q <- q %<+% dd +
  #geom_tiplab(aes(),hjust = -0.2, size=2.5, alpha=.75) +
   geom_cladelab(node=94, label="Influenzavirus", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
   geom_cladelab(node=87, label="Thogotovirus", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
   geom_cladelab(node=98, label="Quaranjavirus (7 segments)", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
   geom_cladelab(node=103, label="Quaranjavirus (8 segments)", align=FALSE,  
                 offset = .8, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5, barsize = 1, fontface = 2) +
   geom_cladelab(node=104, label="Aedes detritus \n orthomyxo-like virus", align=FALSE,  
                 offset = .5, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5) +
   geom_cladelab(node=143, label="	Wuhan mosquito virus 6", align=FALSE,  
  ...
  theme(legend.position= c(0.15, 0.3), legend.text = element_text(size = 15), title = element_text(size = 22)) +
