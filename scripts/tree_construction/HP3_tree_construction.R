...
## reading in tree
tree <- read.tree("HP3_tree.contree") # insert nwk tree with bootstrap
## replacing the label descriptions
replace_words <- read.csv("HP3_references.txt", sep = "\t", header = FALSE, col.names = c("Accession", "Virus"))
...
q <- q %<+% dd +
  ...
  theme(legend.position= c(0.18, 0.7), legend.text = element_text(size = 15), title = element_text(size = 22)) +
