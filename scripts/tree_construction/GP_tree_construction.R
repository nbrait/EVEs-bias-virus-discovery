...
## reading in tree
tree <- read.tree("GP_tree.contree") # insert nwk tree with bootstrap
## replacing the label descriptions
replace_words <- read.csv("GP_references.txt", sep = "\t", header = FALSE, col.names = c("Accession", "Virus"))
...
tree <- root(tree, node = 213, edgelabel = TRUE)
#### converting the bootstrap value outside ggtree
q <- ggtree(tree) + xlim(NA, 5) + expand_limits(y = 130) + coord_cartesian(clip="off") #+ geom_text2(aes(subset=!isTip,label=node), hjust=-.3, size=2) 
show(q)
#q <- flip(q, 330, 493)
...
q <- q %<+% dd +
  ...
  geom_cladelab(node=121, label="Influenzavirus", align=FALSE,  
                offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
  geom_cladelab(node=221, label="Thogotovirus", align=FALSE,  
                offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
  geom_cladelab(node=230, label="Quaranjavirus (7 segments)", align=FALSE,  
                offset = .1, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0, barsize = 1,fontface = 2) +
  geom_cladelab(node=125, label="Quaranjavirus (8 segments)", align=FALSE,  
                offset = .8, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5, barsize = 1, fontface = 2) +
   geom_cladelab(node=183, label="Usinis virus", align=FALSE,  
                 offset = .25, textcolor='black', barcolor='black', angle=0, vjust=0.5, hjust=0) +
   geom_cladelab(node=196, label="Byreska virus", align=FALSE,  
                 offset = .3, textcolor='black', barcolor='black', angle=90, vjust=0.5, hjust=0.5) +
   geom_cladelab(node=156, label="	Culex pipiens \n orthomyxo-like virus ", align=FALSE,  
                 offset = .4, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5) +
   geom_cladelab(node=128, label="Guadeloupe mosquito \n quaranja-like virus 1", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5) +
   geom_cladelab(node=153, label="Isavirus", align=FALSE,  
