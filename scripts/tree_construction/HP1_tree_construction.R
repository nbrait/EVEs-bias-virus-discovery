...
###### HUMAN INPUT ###########
## reading in tree
tree <- read.tree("HP1_tree.contree") # insert nwk tree with bootstrap
## replacing the label descriptions
replace_words <- read.csv("HP1_references.txt", sep = "\t", header = FALSE, col.names = c("Accession", "Virus"))
...
tree <- root(tree, node = 103, edgelabel = TRUE)
...
q <- q %<+% dd +
  ...
  geom_cladelab(node=1, label="Usinis virus", align=FALSE,  
   ...
   geom_cladelab(node=107, label="Guadeloupe mosquito \n quaranja-like virus 1", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black', angle=90, vjust=1, hjust=0.5) +
   geom_cladelab(node=38, label="Insect orthomyxo-like virus 1", align=FALSE,  
                 offset = .1, textcolor='black', barcolor='black',fontface = 1) +
  geom_cladelab(node=66, label="Astopletus virus ", align=FALSE,  
